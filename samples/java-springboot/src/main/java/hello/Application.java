...
public class Application {
  ...
  public String index(@RequestBody ArenaUpdate arenaUpdate) {
    ...
    switch (meState.direction){
      case "N": vector = 1;break;
      case "E": vector = 2;break;
      case "S": vector = 3;break;
      case "W": vector = 4;
    }
  ...
  private PlayerState findClosestBot(PlayerState meState, ArenaUpdate arenaUpdate){
    ...
    List<PlayerState> players = playerStates.stream().filter(v -> !v.equals(meState)).collect(Collectors.toCollection(ArrayList::new));
  ...
  private void getLine(PlayerState meState, PlayerState[] line, int vector) {
    switch (vector){
      case 1: line[0] = new PlayerState(meState.x, meState.y - 1); line[1] = new PlayerState(meState.x, meState.y - 2); line[2] = new PlayerState(meState.x, meState.y - 3); break;
      case 3: line[0] = new PlayerState(meState.x, meState.y + 1); line[1] = new PlayerState(meState.x, meState.y + 2); line[2] = new PlayerState(meState.x, meState.y + 3); break;
      case 2: line[0] = new PlayerState(meState.x + 1, meState.y); line[1] = new PlayerState(meState.x + 2, meState.y); line[2] = new PlayerState(meState.x + 3, meState.y); break;
      case 4: line[0] = new PlayerState(meState.x - 1, meState.y); line[1] = new PlayerState(meState.x + 2, meState.y); line[2] = new PlayerState(meState.x - 3, meState.y); break;
    }
  }
